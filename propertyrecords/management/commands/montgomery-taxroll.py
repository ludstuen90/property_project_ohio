        ...
        with open(abs_file_path, encoding="ISO-8859-1") as csvfile:
            ...
            for number, row in enumerate(reader):
                if number < 229000:
                    ...
                    try:
                        montgomery_county_property_item.legal_acres = Decimal(row["ACRES            "].strip())
                        # print(montgomery_county_property_item.legal_acres )
                    ...
                    # Tax Address
                        ...
                        tax_address.save()
                        montgomery_county_property_item.tax_address = tax_address

                    except:
                        print("EXCEPTION CAUGHT")
                        first_tax_addr = models.TaxAddress.objects.filter(
                            name=row['MAILINGNAME1                            '].strip(),
                            secondary_name=row['MAILINGNAME2                            '].strip(),
                            primary_address_line=row[
                                'PADDR1                                                                                                                   '].strip(),
                            secondary_address_line=row[
                                'PADDR2                                                                          '].strip(),
                        ...
                        montgomery_county_property_item.tax_address = first_tax_addr
